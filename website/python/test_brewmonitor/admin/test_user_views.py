...
class TestAdminViews:
    ...
    def test_add_user(self, admin_client, other_user_data, is_admin):
        if is_admin:
            # checkbox only added to request if ticked
            other_user_data['is_admin'] = 1
        ...
        with bm_config.db_connection() as conn:
            user = find_user(conn, other_user_data['username'])
    ...
    @pytest.mark.parametrize('is_admin', (True, False))
    def test_delete_user(self, admin_client, other_user_data, is_admin):
        bm_config = config_from_client(admin_client.application)
        with bm_config.db_connection() as conn:
            new_user = User.create(conn, is_admin=is_admin, **other_user_data)
